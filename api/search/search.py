...
class Search:
    ...
    async def generate_tages(self, title, extract):
        prompt = PromptTemplate(
            template="Analyze the following Wikipedia article excerpt and generate the five most relevant tags or keywords that summarize its main topics, themes, and concepts. Provide the tags in a single comma-separated list without additional formatting: {input}"
        )
        ...
        clean_tags = [
            tag.strip().title() for tag in tags.split(",") if tag.strip()
        ]  # Strip whitespace, title case, and remove empty entries
